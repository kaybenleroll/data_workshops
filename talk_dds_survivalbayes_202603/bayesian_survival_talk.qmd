...
Can also model $\lambda(t)$ or $\Lambda(t)$
...
- **Quality control**: manufacturing time to failure
- **Customer churn**: time to subscription cancellation
- **Clinical trials**: time to disease recurrence or death
...
$log(T) = \mu + \mathbf{x}^\top \beta + \sigma \epsilon$

\


::: {.left-align}
- $\epsilon$: error term (e.g. Gumbel, Normal, Logistic)
- $\sigma$: scale parameter
- $\exp(\beta)$: **Acceleration Factor (AF)**
:::
...
$$
\begin{align*}
S_1(t) &= S_0\left(\frac{t}{AF}\right) \\
\\
\lambda_1(t) &= \left(\frac{1}{AF}\right) \; \lambda_0 \, \left(\frac{t}{AF}\right)
\end{align*}
$$
...
## Fragility Example: Survival

```{r aft_data_prep}
#| echo: false

lambda <- 10
t_seq  <- seq(0.1, 30, length.out = 200)

aft_fragility_data_tbl <- expand_grid(
    t  = t_seq,
    AF = c(0.5, 1, 2)
    ) |>
  mutate(
    group    = factor(
      AF,
      levels = c(0.5, 1, 2),
      labels = c("AF=0.5 (Accelerated)", "Baseline (AF=1)", "AF=2 (Decelerated)")
      ),
    k        = 1.5,
    survival = exp(-(t / (lambda * AF))^k),
    hazard   = (k / (lambda * AF)) * (t / (lambda * AF))^(k - 1),
    log_t    = log(t),
    density  = dnorm(log_t, mean = log(10) + log(AF), sd = 0.5)
    )

aft_infant_data_tbl <- expand_grid(
    t  = t_seq,
    AF = c(0.5, 1, 2)
    ) |>
  mutate(
    ...
    hazard   = (k / (lambda * AF)) * (t / (lambda * AF))^(k - 1),
    log_t    = log(t),
    density  = dnorm(log_t, mean = log(10) + log(AF), sd = 1.0)
    )
```
...
aft_fragility_data_tbl |>
  ggplot(aes(x = t, y = survival, color = group)) +
  geom_line(linewidth = 1.2) +
  labs(
    x     = "Time",
    y     = "Survival S(t)",
    color = "Condition",
    title = "Visualising Survival Times (Fragility / Wear-out)"
    )
```

## Fragility Example: Hazard
...
aft_fragility_data_tbl |>
  ggplot(aes(x = t, y = hazard, color = group)) +
  geom_line(linewidth = 1.2) +
  labs(
    x     = "Time",
    y     = "Hazard h(t)",
    color = "Condition",
    title = "Visualising Hazards (Fragility / Wear-out)"
    )
```

## Fragility Example: Log-Time
...
aft_fragility_data_tbl |>
  ggplot(aes(x = log_t, y = density, fill = group)) +
  geom_area(alpha = 0.5, color = "white") +
  labs(
    x    = "log(Time)",
    y    = "Density",
    fill = "Condition",
    title = "Density Estimates of Time-to-Failure (Fragility)"
    )
```
...
aft_infant_data_tbl |>
  ...
  labs(
    ...
    y     = "Survival S(t)",
    color = "Condition",
    title = "Visualising Survival Times (Infant Mortality)"
    )
```
...
aft_infant_data_tbl |>
  ...
  labs(
    ...
    y     = "Hazard h(t)",
    color = "Condition",
    title = "Visualising Hazards (Infant Mortality)"
    )
```

## Infant Mortality: Log-Time
...
aft_infant_data_tbl |>
  ...
  labs(
    ...
    y    = "Density",
    fill = "Condition",
    title = "Density Estimates of Time-to-Failure (Infant Mortality)"
    )
```

## Interpreting AF and Risk

\


AF scales **time**, not **hazard**

\


($AF > 1 \implies$ longer life).
...
Hazard is lower at *all* times.

\


**Infant Mortality ($k < 1$)**

Early hazard is *higher*, late hazard is *lower*.
...
AFT handles this naturally, Proportional Hazards fails.
...
L(\beta) = \prod_{i \in \mathcal{D}}
  \frac{\exp(\mathbf{x}_i^\top \beta)}
       {\sum_{j \in R(t_i)} \exp(\mathbf{x}_j^\top \beta)}
$$
