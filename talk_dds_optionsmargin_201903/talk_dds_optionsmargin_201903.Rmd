...
import::from(dplyr      ,mutate, filter, select, group_by, slice, ungroup
                        ...
                        ,pull, if_else, count, arrange, bind_rows, mutate_all)
...
moneyness_tbl <- tribble(
            ~moneyness,     ~S
   ,"Out of the Money",  99.50
   ,    "At the Money", 100.00
   ,    "In the Money", 100.50
)

option_decay_tbl <- crossing(moneyness = moneyness_tbl$moneyness, T = (1:40) / 252) %>%
    inner_join(moneyness_tbl, by = 'moneyness') %>%
    mutate(price = map2_dbl(S, T, calc_call_price)
          ,days_passed = 40 - (252 * T)
           ) %>%
    arrange(S, days_passed) %>%
