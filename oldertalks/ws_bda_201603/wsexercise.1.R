...
qplot(false.alarm.seq, infected.prob, geom = 'line', ylim = c(0, 1))
...
twotest.infected.prob <- sapply(false.alarm.seq, function(iter.fa) {
    data.dt <- generate.disease.twotest.data(n             = 100000
                                            ,prior.prob    = 0.001
                                            ,hit.rate.1    = 0.99
...
qplot(false.alarm.seq, twotest.infected.prob, geom = 'line', ylim =c(0, 1))


qplot(false.alarm.seq, infected.prob, geom = 'line', ylim = c(0, 1)) +
    geom_line(aes(y = twotest.infected.prob), colour = 'red') +
    xlab("False Alarm Probability") +
    ylab("Infected Probability")
