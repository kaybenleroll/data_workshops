...
\item Incorporate different $\omega$ and $\theta$ priors
\item Generate fake data to try new approaches (change-point for example)
\item Add hierarchy of product lines to model
