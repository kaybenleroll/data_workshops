...
\pagebreak
\worksheetsection{Expanding the Hierarchy: Multiple Categories}

\noindent
While the methods employed in the previous section work fine, a more
`Bayesian' approach would be to incorporate the category data into the
...
\worksheetbreak
Incorporating the category data for each coin is relatively simple --- we
just need to index by $\mu$ and $\kappa$ for each category, as shown in
Figure \ref{fig2}.
...
\caption{\label{fig2}
Graphical Representation of the Multiple Category, Multiple Tests Hierarchical
model}
\end{center}
\end{figure}
...
\worksheetbreak
For the purposes of estimating and inferring on category-quality, we are
...
\texttt{generate\_multiple\_hier\_trial\_data} for 5 categories. You
...
\texttt{multicat\_lognormal\_k.stan}, which puts a lognormal prior
