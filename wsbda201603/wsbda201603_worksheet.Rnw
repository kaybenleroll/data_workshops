...
\pagebreak
\worksheetsection{Markov-Chain Monte Carlo (MCMC) for Hierarchical Models}
...
\worksheetbreak
Instead, we will use a technique known as Markov-Chain Monte Carlo to
sample from the posterior distribution. To do this, we use Stan, an
...
which involves specifying the model used by Stan. This is usually done
...
\texttt{sample\_rstan\_template.R}. Use this file as a template.
...
%%%
%%%
%%% SECTION: Using the Hierarchical Model in rstan
%%%
%%%

\pagebreak
\worksheetsection{Using the Hierarchical Model in rstan}

\noindent
We build our first Stan model based off the data from a single
...
\noindent
One issue with running Stan models is that it can take time. To help
...
%%%
%%%
%%% SECTION: Expanding the Model to Compare Mints
%%%
%%%
...
%%%
%%%
%%% SECTION: Exploring the Effect of the Data
%%%
%%%
