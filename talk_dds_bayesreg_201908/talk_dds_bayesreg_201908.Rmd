...
diamondprice_stanlm %>%
  plot('trace') + ggtitle("Parameter Traceplots")
```
...
diamondprice_stanlm %>%
  plot('hist') + ggtitle("Parameter Histograms")
```
...
sample_fitted_tbl <- sample_12_tbl %>%
  add_fitted_draws(diamondprice_stanlm)
  
ggplot(sample_fitted_tbl) +
...
ggplot(plot_tbl %>% filter(sample_count <= 5)) +
  geom_point(aes(x = county, y = mean_val)) +
  geom_errorbar(aes(x = county, ymin = lower, ymax = upper, colour = size), width = 0) +
  scale_colour_gradient(low = 'blue', high = 'red') +
  xlab("County") +
