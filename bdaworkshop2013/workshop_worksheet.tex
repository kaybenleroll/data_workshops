...
us extend the above scenario in more interesting ways.

\begin{itemize}
\item What if we have multiple coins minted from the same mint?
\item How can we use results from different coins to get inferences on the mint?
\item What types of inferential models can be built from this set up?
\end{itemize}
...
counts of coin tosses for each coin. Use different values of $\theta$
for each coin, but have them be relatively similar.
