...
# BTYD Customer Visualisations

The concepts around BTYD modelling suggest a few other ways to look at the
data, so we use these now.

To do this, we need to aggregate our data down to both the invoice and the
daily spend level - a fairly straightforward manipulation.


```{r calculate_customer_daily_spend, echo=TRUE}
daily_spend_invoice_tbl <- cleaned_data_tbl %>%
  drop_na(customer_id) %>%
  filter(
    quantity > 0,
    exclude == FALSE
...
customer_firstdate_tbl %>% write_rds("data/customer_cohort_data_tbl.rds")
```
