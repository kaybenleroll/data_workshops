...
screen_stock_code <- c(
  ...
  "AMAZONFEE", "ADJUST", "TEST001", "TEST002", "ADJUST2"
  )
...
This data requires us to exclude a number of rows from the dataset including
all the data with no `customer_id`.
...
We now construct the by-customer daily spend summary tables.

```{r construct_customer_summary_tables, echo=TRUE}
...
daily_spend_customer_tbl <- daily_spend_invoice_tbl %>%
...
daily_spend_customer_tbl %>% glimpse()
```
...
```{r construct_customer_cohort, echo=TRUE}
customer_cohort_tbl <- tnx_purchase_tbl %>%
  group_by(customer_id) %>%
  summarise(
    .groups = "drop",
...
customer_tnx_diffs_tbl <- daily_spend_customer_tbl %>%
  group_by(customer_id) %>%
  summarise(
    .groups = "drop",
...
plot_tbl <- daily_spend_customer_tbl %>%
...
cohort_dropout_est_tbl <- customer_cohort_tbl %>%
  ...
  inner_join(daily_spend_customer_tbl, by = "customer_id") %>%
...
# Construct Lookup Dataset and Export

In this final section we construct a number of different datasets for use in
the project and then write the data to disk so they can be used in subsequent
workbooks.
...
```{r dedupe_stock_code_descriptions, echo=TRUE}
dedupe_stock_code_descriptions <- function(stock_desc) {
  dedupe_descs <- stock_desc %>%
    enframe(name = NULL, value = "stock_desc") %>%
    mutate(
      stock_desc   = stock_desc %>% str_trim() %>% str_squish(),
      desc_length  = stock_desc %>% nchar(),
      trimmed_desc = stock_desc %>% str_replace_all(" ", "")
      ) %>%
    group_by(trimmed_desc) %>%
    slice_max(order_by = desc_length, n = 1, with_ties = FALSE) %>%
    ungroup() %>%
    pull(stock_desc)
...
stock_code_lookup_tbl <- cleaned_data_tbl %>%
  ...
  summarise(
    ...
    desc = description %>%
      dedupe_stock_code_descriptions() %>%
      sort() %>%
      str_c(collapse = " : ")
    ) %>%
...
tnx_purchase_tbl         %>% write_rds("data/tnx_purchase_tbl.rds")
...
daily_spend_customer_tbl %>% write_rds("data/daily_spend_customer_tbl.rds")
```
