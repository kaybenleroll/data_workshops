...
```{r generate_pnbd_extdata_hiermu_validation_sims, echo=TRUE}
precompute_dir <- "precompute/pnbd_extdata_hiermu"
...
pnbd_extdata_hiermu_validsims_lookup_tbl <- pnbd_extdata_hiermu_validation_tbl %>%
  group_nest(customer_id, .key = "cust_params") %>%
  mutate(
    sim_file = glue(
      "{precompute_dir}/sims_pnbd_extdata_hiermu_{customer_id}.rds"
      )
    )
    

exec_tbl <-  pnbd_extdata_hiermu_validsims_lookup_tbl %>%
  anti_join(precomputed_tbl, by = "sim_file")
...
pnbd_extdata_hiermu_validsims_lookup_tbl %>% glimpse()
```

We now load all the simulations into a file.

```{r load_pnbd_extdata_hiermu_validsims, echo=TRUE, cache=TRUE}
pnbd_extdata_hiermu_validsims_tbl <- pnbd_extdata_hiermu_validsims_lookup_tbl %>%
  mutate(
    data = map(sim_file, ~ .x %>% read_rds() %>% select(draw_id, sim_tnx_count, sim_tnx_last))
    ) %>%
  select(customer_id, data) %>%
...
pnbd_extdata_hiermu_validsims_tbl %>% glimpse()
```


```{r construct_pnbd_extdata_hiermu_tnx_simulations, echo=TRUE}
tnx_data_tbl <- btyd_obs_stats_tbl %>% 
  semi_join(pnbd_extdata_hiermu_validsims_tbl, by = "customer_id")

obs_customer_count  <- tnx_data_tbl %>% nrow()
obs_total_tnx_count <- tnx_data_tbl %>% pull(tnx_count) %>% sum()

pnbd_extdata_hiermu_tnx_simsumm_tbl <- pnbd_extdata_hiermu_validsims_tbl %>%
  group_by(draw_id) %>%
  summarise(
    .groups = "drop",
...
ggplot(pnbd_extdata_hiermu_tnx_simsumm_tbl) +
  geom_histogram(aes(x = sim_customer_count), binwidth = 10) +
  geom_vline(aes(xintercept = obs_customer_count), colour = "red") +
  labs(
    x = "Simulated Customers With Transactions",
