...
pnbd_fitdata_tbl <- customer_transactions_tbl %>%
  calculate_transaction_cbs_data(last_date = as.POSIXct("2011-04-01"))

pnbd_fitdata_tbl %>% glimpse()
```
...
obs_stats_tbl <- customer_transactions_tbl %>%
  filter(
...
stan_data_lst <- pnbd_fitdata_tbl %>%
...
pnbd_extdata_fixed_validation_tbl <- pnbd_extdata_fixed_stanfit %>%
  recover_types(pnbd_fitdata_tbl) %>%
...
pnbd_extdata_fixed_validsims_tbl <- pnbd_extdata_fixed_validation_tbl %>%
  group_nest(customer_id, .key = "cust_params") %>%
  mutate(
    sim_file = glue(
      "precompute/pnbd_extdata_fixed/sims_pnbd_extdata_fixed_{customer_id}.rds"
      ),
