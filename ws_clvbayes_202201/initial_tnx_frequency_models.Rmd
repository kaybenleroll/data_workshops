...
customer_cohort_tbl <- read_rds("data/synthdata_longframe_cohort_tbl.rds")
...
freqmodel_flat_brmfit <- brm(
  formula = btyd_count ~ 0 + offset(log(tnx_weeks)) + customer_id,
  data    = fit_data_tbl,
  family  = poisson(),
  prior   = c(
...
freqmodel_flat_postcheck_tbl %>% glimpse()
```
