...
We now turn our attention to validating our model fit.
...
the remained of the time period. This should help us validate the quality of
our model.
...
training_stats_tbl <- training_tbl %>% calculate_transaction_summary_stats()
...
```{r fit_flat_model_subset, echo=TRUE}
stan_modelname <- "freqmodel_flat_subset"

stan_data_lst <- training_stats_tbl %>%
  transmute(customer_id, btyd_count, tnx_weeks = full_weeks) %>%
  compose_data()
