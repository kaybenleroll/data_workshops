...
obs_2019_id1000_stats_tbl <- customer_transactions_tbl %>%
  filter(
...
obs_2019_id1000_stats_tbl %>% glimpse()
```
...
```{r construct_simulation_comparisons_pnbd_fixed_fixed2}
...
To avoid those low-end values of the model, we refit our model with a lower
coefficient of variation -- this should solve the lower end of the distribution
though the tradeoff means that that the right tail of the distribution is less
pronounced.
...
One thing to look at is to compare the distribution of total transaction count
for each iteration in our posterior sample against the observed amount. This
should at least give us a broad sense of how well the model is capturing that
aspect of the data as a whole.
