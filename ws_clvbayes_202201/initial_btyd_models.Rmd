...
```{r}
customer_basic_parameters_tbl <- read_rds("data/customer_basic_parameters_tbl.rds")
...
calculate_transaction_cbs_data <- function(tnx_data_tbl, last_date) {
  cbs_data_tbl <- tnx_data_tbl %>%
    filter(tnx_date <= last_date) %>%
    group_by(customer_id) %>%
    summarise(
      .groups = "drop",

      first_tnx_date = min(tnx_date),
      last_tnx_date  = max(tnx_date),
...
calculate_posterior_data_probs <- customer_basic_parameters_tbl %>%
