...
```{r fit_basic_fixedcov1_stan_model, echo=TRUE, warning=TRUE, cache=TRUE}
stan_data_lst <- compose_data(
  customer_basic_pnbd_cbs_lst,
...
pnbd_basic_fixedcov1_stanfit <- sampling(
...
pnbd_basic_fixedcov1_stanfit %>% summary(pars = c("lb_mean", "mu_mean"))
```

We now refit the model with a higher level of coefficient-of-variance.

```{r fit_basic_fixedcov2_stan_model, echo=TRUE, warning=TRUE, cache=TRUE}
stan_data_lst <- compose_data(
  customer_basic_pnbd_cbs_lst,
...
pnbd_basic_fixedcov2_stanfit <- sampling(
  pnbd_basic_fixedcov_stanmodel,
  data    = stan_data_lst,
  chains  =    4,
  warmup  = 1000,
...
pnbd_basic_fixedcov2_stanfit %>% summary(pars = c("lb_mean", "mu_mean"))
```
...
pnbd_basic_fixedcov2_stanfit %>% check_hmc_diagnostics()

pnbd_basic_fixedcov2_stanfit %>% get_bfmi() %>% round(4)
```
...
post10_customer_tbl <- pnbd_basic_fixedcov1_stanfit %>%
  recover_types(customer_basic_pnbd_cbs_tbl) %>%
  spread_draws(mu[customer_id], lambda[customer_id]) %>%
  ungroup()


post20_customer_tbl <- pnbd_basic_fixedcov2_stanfit %>%
  recover_types(customer_basic_pnbd_cbs_tbl) %>%
  spread_draws(mu[customer_id], lambda[customer_id]) %>%
  ungroup()
