...
We use the provided tools from the `CLVTools` package to convert our
transaction data into the required formats.
...
We start by just fitting the lifetime model, working on the basis that we 
cannot observe lifetimes directly so instead we work on the censored
observations indicated by the times of transactions.

```{r construct_exponential_lifetime_model, echo=TRUE}
pnbd_lifetime_fixed_stanmodel <- stan_model(
  "stan_code/clv_pnbd_intro_lifetime_fixed.stan",
  model_name = "pnbd_lifetime_model",
  verbose    = TRUE
  )
```


```{r fit_lifetime_fixed_stan_model, echo=TRUE, cache=TRUE}
...
pnbd_lifetime_fixed_stanfit <- sampling(
  pnbd_lifetime_fixed_stanmodel,
  data   = stan_data_lst,
  chains = 4,
  iter   = 1000,
  seed   = 42
...
pnbd_lifetime_fixed_stanfit %>% summary()
```
