...
match_last_quantity <- ~ .x %>%
  arrange(desc(invoice_dttm)) %>%
  head(1) %>%
  select(orig_row_id = row_id)
...
returns_lookup_tbl <- returns_data_tbl %>%
  select(return_row_id = row_id, last_match) %>%
  unnest(last_match)
