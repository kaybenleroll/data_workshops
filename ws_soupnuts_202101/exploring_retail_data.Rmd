...
We start by looking at all the transactions with a negative quantity and for 
each item returned, we create a list of all previous purchases to match against
it.

```{r create_matching_returns_data, echo=TRUE}
filter_returns_transactions <- function(use_stock, use_customer, use_quantity,
                                use_dttm, data_tbl) {
...
returns_data_tbl <- data_tbl %>%
  filter(quantity < 0) %>%
  select(stock_code, customer_id, quantity, invoice_dttm) %>%
  mutate(
    returns = pmap(
