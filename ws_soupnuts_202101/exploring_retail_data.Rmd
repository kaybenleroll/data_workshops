...
for (plot_varname in numeric_vars) {
  ...
  neg_data_tbl <- explore_data_tbl %>%
    ...
    mutate(!! plot_varname := abs(!! sym(plot_varname)))
  ...
  explore_pos_log_plot <- explore_plot +
    scale_x_log10(labels = label_comma()) +
    ggtitle("Positive Values")
  
  explore_neg_log_plot <- explore_pos_log_plot %+%
    neg_data_tbl +
  ...
  plot_grid(
      explore_std_plot,
      NULL,
      explore_pos_log_plot,
      explore_neg_log_plot,
      nrow = 2
      ) %>%
    print()
}
...
# 
# 
# basket_graph <- graph_from_data_frame(
#   d        = edges_tbl,
#   directed = FALSE,
#   vertices = nodes_tbl
#   )
