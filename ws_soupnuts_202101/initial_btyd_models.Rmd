...
Similar to the BG/NBD model, the P/NBD model relies on five assumptions:
  ...
  distribution $\Gamma(\tau \, | \, s, \beta)$ with shape parameter $s$ and
  rate parameter $\beta$.
...
predict_rates_tbl <- predict_bgnbd_tbl %>%
  mutate(
...
predict_r     <- params_bgnbd_tbl %>% filter(parameter == "r")     %>% pull(mean)
predict_alpha <- params_bgnbd_tbl %>% filter(parameter == "alpha") %>% pull(mean)

sim_rates <- predict_rates %>% length() %>% rgamma(shape = predict_r, rate = predict_alpha)

create_summ_table <- function(x) {
  x %>% summary() %>% enframe() %>% mutate(value = as.numeric(value))
}
...
construct_split_bgnbd_estimates <- function(tnx_splits, cache_file,
                                            cutoff_date) {
