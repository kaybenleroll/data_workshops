...
```{r plot_beta_new_england, echo=FALSE}
beta_new_england <- dbeta(theta_seq, 1+2, 1+6)

ggplot() +
    geom_line(aes(x = theta_seq, y = beta_new_england)) +
    geom_line(aes(x = theta_seq, y = beta_england), colour = 'blue') +
    xlab(expression(theta)) +
    ylab("Density") +
    ggtitle("Updated Posterior Distribution for World Cup Win (uniform prior)")
```
