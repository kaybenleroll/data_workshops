...
claimant_loss_plot <- ggplot(autobi_tbl) +
  geom_boxplot(aes(x = claimant_insured, y = log_loss)) +
  scale_y_continuous(labels = comma) +
...
if(!file_exists(loglik_heatmap_data_file)) {
  intcpt_vals <- seq(-10.0, 10.0, length.out = 250)
  male_vals   <- seq(-10.0, 10.0, length.out = 250)
  
  loglik_heatmap_tbl <- crossing(intcpt = intcpt_vals, male = male_vals) %>%
    mutate(loglik = map2_dbl(intcpt, male,
                             loss_loglik_func,
                             fit_sd = 1.5, show_prob = 0.001))
...
autobi_stanlm <- stan_lm(
  log_loss ~ attorney + male + marital + claimant_insured + seatbelt,
  data            = autobi_tbl,
